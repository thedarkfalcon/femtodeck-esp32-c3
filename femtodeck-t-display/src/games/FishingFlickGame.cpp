...
void void FishingFlickGame::drawRunning(TFT_eSPI& tft) { tft.fillScreen(TFT_BLACK); { tft.fillScreen(TFT_BLACK);
  tft.drawRect(, 0, width, height);
  drawWater(tft);


  if (fishState_ == FishState::Waiting) {
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(18, 13, "Waiting");
    tft.setCursor(3, 7);
    tft.print("L");
    tft.print(level_);
    tft.print(" ");
    tft.print(fishWeight_);
    tft.print("g");
    tft.drawLine(34, 12, 34, 24);
    tft.drawPixel(34, 25);
    return;
  }

  if (fishState_ == FishState::Hooking) {
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(18, 13, "TUG!");
    const int tugOffset = ((millis() / 120) % 2 == 0) ? 3 : -3;
    tft.drawLine(34, 12, 34 + tugOffset, 24);
    tft.drawPixel(34 + tugOffset, 25);
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(18, 36, "Tap!");
    return;
  }

  if (fishState_ == FishState::Caught) {

    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(14, 17, "Caught!");

    tft.setCursor(18, 28);
    tft.print(caughtWeight_);
    tft.print("g");
    return;
  }
  ...
  tft.print(level_);
  tft.print(" ");
  tft.print(fighting_ ? "FIGHT" : "REEL");
  tft.drawRect(5, 13, 58, 5);
  tft.fillRect(6, 14, static_cast<int>(progress_ * 0.56f), 3);
  tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(5, 27, "T");
  tft.drawRect(14, 22, 49, 5);
  tft.fillRect(15, 23, static_cast<int>(min(47.0f, tension_ * 0.47f)), 3);
  if (tension_ > 78.0f) {
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(22, 36, "LINE!");
  } else if (tension_ < 15.0f) {
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(22, 36, "SLACK!");
  }
}
...
void FishingFlickGame::drawStart(TFT_eSPI& tft) { tft.fillScreen(TFT_BLACK);
  ...
  if (showStartPromptPage()) {
    ...
    tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(3, 10, "Top Fish");
    ...
    if (bestWeight_ == 0) {
      ...
      tft.print(initials);
      tft.print(" ");
      tft.print(bestWeight_);
      tft.print("g");
    }
  } else {
    drawWater(tft);
    tft.drawLine(12, 9, 24, 25);
    tft.drawLine(24, 25, 31, 24);
    tft.drawPixel(33, 25);
...
void FishingFlickGame::drawEnd(TFT_eSPI& tft) { tft.fillScreen(TFT_BLACK);
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.drawString(3, 9, title);
...
void FishingFlickGame::drawWater(TFT_eSPI& tft) {
  tft.drawLine(1, 29, width - 2, 29);
  for (uint8_t i = 0; i < 5; i++) {
    const int x = 4 + i * 14;
    tft.drawLine(x, 33, x + 5, 31);
    tft.drawLine(x + 5, 31, x + 10, 33);
  }
}
